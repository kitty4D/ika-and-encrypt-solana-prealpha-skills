...
**CLI:** `node skills/encrypt-solana-prealpha/scripts/audit-encrypt-solana-prealpha.mjs [--force] [--root=DIR]` from the **skill package** repository root (the tree that contains `skills/encrypt-solana-prealpha/`), or `node scripts/audit-encrypt-solana-prealpha.mjs` with cwd = this skill folder (`--root` defaults to `process.cwd()`).
