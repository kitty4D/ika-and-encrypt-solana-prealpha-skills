 ...
 * From skill folder: node scripts/audit-encrypt-solana-prealpha.mjs [--force] [--no-drift] [--drift=strict] [--root=DIR]
...
const NPM_PACKAGES = ["@encrypt.xyz/pre-alpha-solana-client", "@solana/kit"];

const httpsJson = httpsJsonDefault("encrypt-solana-prealpha-audit-script");
...
async function printNpmSdkOutdated(depRows) {
  ...
  for (const row of depRows) {
    ...
    for (const pkgName of NPM_PACKAGES) {
      const specKey = pkgName === "@encrypt.xyz/pre-alpha-solana-client" ? "encryptSpec" : "kitSpec";
      const hasKey = pkgName === "@encrypt.xyz/pre-alpha-solana-client" ? "encrypt" : "kit";
  ...
  if (!any) {
    console.log("(no package.json with @encrypt.xyz/pre-alpha-solana-client or @solana/kit under scan root)");
  }
  console.log("");
...
function printFollowUpActions(opts) {
  ...
  if (stale) {
    lines.push(`Refresh skill: bump docs-revision.md to match encrypt-pre-alpha main (see compare) or pause using this skill until updated — ${compareUrl}`);
  }
  if (stale && usedForce) {
    lines.push("You used --force while docs/ was stale: re-check flows against the live book; skill prose may lag.");
  }
  if (mismatchCount > 0) {
    lines.push(`Fix ${mismatchCount} canonical literal issue(s) above (Encrypt gRPC URL or full program id vs SKILL.md).`);
  }
  ...
  lines.push("Manual pass: trace CreateInput / ReadCiphertext, execute_graph, fees & events per flows.md + grpc-api.md + gotchas.md; cite file:line.");
  ...
  for (const f of walkFiles(root, 8, DEFAULT_SKIP)) {
    ...
    if (/encrypt\.ika-network|pre-alpha-dev-1\.encrypt/i.test(text)) {
      if (!text.includes(canonical.grpc)) {
        mismatches.push({ rel, kind: "mentions Encrypt gRPC host but not the canonical URL from SKILL.md" });
      }
    }
    if (text.includes("4ebfzWdKnrnGseuQ") && !text.includes(canonical.programId)) {
      mismatches.push({ rel, kind: "contains 4ebfzWdKnrnGseuQ… fragment but not full canonical program id string" });
    }
  }
...
async function main() {
  ...
  if (stale && force) {
    console.warn("WARNING: skill docs pin is behind encrypt-pre-alpha main for docs/; results may not match current book.");
    console.warn("");
  }
  ...
  const isBundleSelf = isSkillBundleRoot(root, "encrypt-solana-prealpha");
  if (isBundleSelf) {
    console.log(
      "(scan root looks like the encrypt skill bundle itself - excluding tests/fixtures/ and drift-rules.mjs from literal/drift scans)",
    );
    console.log("");
  }
  ...
  if (!depReport.length) {
    ...
    for (const r of depReport) {
      console.log(`${r.file}: @encrypt.xyz/pre-alpha-solana-client=${r.encrypt} @solana/kit=${r.kit}`);
    }
  }
  console.log("");
  ...
  if (driftMode !== "off") {
    ...
    const findings = scanProjectForDrift({
      root,
      rules: DRIFT_RULES,
      skipBasenames: new Set(["audit-encrypt-solana-prealpha.mjs", "drift-rules.mjs"]),
