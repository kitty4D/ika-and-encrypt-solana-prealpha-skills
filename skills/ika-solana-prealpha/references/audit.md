...
Load when this skill is invoked with **`audit`** (e.g. `/ika-solana-prealpha audit`), **`audit-force`**, **`audit-fix`**, or you need the full gate / script / checklist after changing [`docs-revision.md`](docs-revision.md).

## Slash commands and CLI

**Slash:** `/ika-solana-prealpha audit` (hard stop if the skill's `docs/` pin is stale) or `audit-force` (warn, then continue). `/ika-solana-prealpha audit-fix <rule-id>` applies the fix for a specific drift rule id reported by a prior run.

**CLI:** `node skills/ika-solana-prealpha/scripts/audit-ika-solana-prealpha.mjs [--force] [--no-drift] [--drift=strict] [--root=DIR]` from the **skill package** repository root (the tree that contains `skills/ika-solana-prealpha/`), or `node scripts/audit-ika-solana-prealpha.mjs` with cwd = this skill folder (`--root` defaults to `process.cwd()`).
...
Catalog lives in [`drift-rules.mjs`](drift-rules.mjs) (sibling of this doc). Each rule has a stable `id`, a `severity` (critical / high / medium / low), a `since` date, and an `evidence` citation. Add a rule whenever `CHANGELOG-IKA.md` records a conceptual shift.
...
3. **Semantic checklist (user code):** With [`flows.md`](flows.md), [`grpc-api.md`](grpc-api.md), and [`account-layouts.md`](account-layouts.md), trace **`Sign` / `SubmitTransaction`**, **`ApproveMessage` / CPI**, **`MessageApproval`** reads, and **flow 6** verification; cite **file:line**. Note gaps; do not invent upstream APIs. **Finish** by giving the user a **numbered follow-up list** (combine script output with [Conclusion](#conclusion-actionable-follow-ups) below — they choose what to do).
...
1. **Skill bundle stale (`docs/` vs `main`):** Bump [`docs-revision.md`](docs-revision.md) to current [`ika-pre-alpha`](https://github.com/dwallet-labs/ika-pre-alpha) `main`, refresh affected `references/`, re-run `audit-ika-solana-prealpha.mjs` until `docs/ vs main: fresh` — or **stop using** this skill until updated.
...
4. **Drift block (skill-vs-codebase):** For each `critical` / `high` finding, paste the rule's `fix:` prompt back to the skill (e.g. `/ika-solana-prealpha audit-fix <rule-id>`). The skill will open the cited `references/` section and confirm before changing code. `NEW since your last sync:` ids are the most important — they appeared after the last run wrote `.skill-audit.json`.
5. **npm lockfile vs `latest`:** If the script reported **OUTDATED**, consider upgrading `@ika.xyz/pre-alpha-solana-client` / `@solana/kit` after reading release notes; if **no lockfile resolved**, run install or point `--root` at the app package.
6. **Semantic pass (always worth doing):** Trace `Sign` / `SubmitTransaction`, `ApproveMessage` / CPI, `MessageApproval` reads, flow 6 verification — [`flows.md`](flows.md), [`grpc-api.md`](grpc-api.md), [`account-layouts.md`](account-layouts.md); note **file:line** gaps.
7. **Runtime confidence:** Run unit/integration tests and a small **devnet** smoke path if you touched ika client or CPI code.

**Gate-only stop (exit 2, no `--force`):** Still leave the user with items **1** (refresh skill or pause using it) and optionally **7** for unrelated work — plus the compare URL from stderr/script.
